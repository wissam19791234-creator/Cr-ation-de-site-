...
const REQUIRED_STILLS = [15, 75, 135, 195, 255, 330, 405, 435];
...
const res = spawnSync(ffmpeg, ['-y','-framerate',String(FPS),'-i',join(frameDir,'frame-%04d.png'),'-c:v','libx264','-crf','18','-pix_fmt','yuv420p','-movflags','+faststart',output], {stdio:'inherit'});
if (res.status !== 0) process.exit(res.status ?? 1);
for (const still of REQUIRED_STILLS) {
  console.log(`Still vérifié: out/frames/frame-${String(still).padStart(4, '0')}.png`);
}
